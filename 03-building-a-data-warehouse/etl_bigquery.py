...
if __name__ == "__main__":
    ...
    with open("github_events.csv", "w") as csv_file:
        ...
        writer.writerow(["id", "type", "actor", "repo", "created_at"])  # แก้ไขตรงนี้

        for datafile in all_files:
            with open(datafile, "r") as f:
                data = json.loads(f.read())
                for each in data:
                    writer.writerow([each["id"], each["type"], each["actor"]["login"], each["repo"]["name"], each["created_at"]])  # แก้ไขตรงนี้

    main(dataset_id="GitHub", table_id="events", file_path="github_events.csv")  # แก้ไขตรงนี้
